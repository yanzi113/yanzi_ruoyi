<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="药品类型" prop="drugType">
        <el-select v-model="queryParams.drugType" placeholder="请选择药品类型" clearable>
          <el-option
            v-for="dict in dict.type.pill_drug_datatype"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="drugList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="厂家名称" align="center" prop="pillFactory.factoryName" />
      ...
      <el-table-column label="药品类型" align="center" prop="drugType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.pill_drug_datatype" :value="scope.row.drugType"/>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="生产厂家" prop="factoryId">
          <el-select v-model="form.factoryId" placeholder="请输入生产厂家">
            <el-option
...
export default {
  name: "Drug",
  dicts: ['pill_drug_pt', 'pill_drug_datatype', 'sys_normal_disable'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    /** 重置按钮操作 */
    resetQuery() {
      this.queryParams['pillFactory.factoryName'] = null;
